...
`runtime-verify` is a read-only static game-effect verifier. It checks that imported portal targets resolve to imported map scenes and that `CoreEngine` runtime scripts expose the expected portal-to-room-loading chain (`Portal.gd` target_map/target_area -> `TYPE_LOAD_ROOM_REQUEST` -> `RoomFlowActor.gd` -> `RoomFlowRouter.gd` -> `RoomFlowIntentExecutor.gd` -> `Game.load_room`). It also verifies MapEditor-owned map data seams: collision metadata paths under `CoreEngine/Maps/Resources/<Map>/collision_*.json`, collision JSON shape, TileMapLayer `tile_map_data`, portal animation fields (`portal_anim_dir`, `portal_anim_fps`, `portal_upscale`), template-room texture fields, ordinary-room `BackgroundLayer/BackgroundTexture` and `ForegroundTextureLayer/ForegroundTexture` writeback, texture transform metadata consumers, foreground collision runtime surface, and `GEN/...` generated room factory wiring. Add `--summary` for a human-readable Testor report. It does not execute a live player transition.
