...
func load_room(path: String):
	if map_changing:
		push_warning("MetSysGame.load_room ignored because map_changing=true. requested=%s resolved=%s" % [path, ResourceUID.uid_to_path(path) if path.begins_with("uid://") else path])
		return
	
	map_changing = true
	var resolved := ResourceUID.uid_to_path(path) if path.begins_with("uid://") else path
	print("MetSysGame.load_room begin requested=%s resolved=%s" % [path, resolved])
	...
	map = _load_room(path)
	if map == null:
		push_error("MetSysGame.load_room failed to instantiate map. requested=%s resolved=%s" % [path, resolved])
		map_changing = false
		return
	add_child(map)
	...
	map_changing = false
	print("MetSysGame.load_room end scene_file_path=%s room_id=%s" % [str(map.scene_file_path), str(MetSys.get_current_room_id())])
	room_loaded.emit()
